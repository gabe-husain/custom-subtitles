document.addEventListener('DOMContentLoaded', function() {
    chrome.runtime.sendMessage({ action: 'initializeOverlay' }, async (response) => {
        if (!response || !response.success) {
            console.error('Overlay initialization failed:', response?.error);
            document.getElementById('status').textContent = 
                chrome.i18n.getMessage('errorInitializing');
            return;
        }
